...
def _print_tree_view(cat_uri: str, cat: Any) -> None:
    ...
    Only covers what we need for sanity-checking a sync: namespace and
    table listing. Falls back to a flat table list if list_tables()'s
    return shape differs from what we expect.
    """

    print(f"\U0001f917\U0001f9ca Catalog: {cat_uri}\n")
    print("carbon (namespace)")

    try:
        tables = list(cat.list_tables("carbon"))
    ...
    for i, table in enumerate(tables):
        ...
        print(f"{connector} {table}")


def sync_from_hub(catalog_id: str, *, tree_view: bool = False) -> None:
    """Patched equivalent of: faceberg <catalog_id> sync [--tree-view]

    Runs in this process, so the write_utf8 / to_bytes / write_manifest
    monkeypatches above are already active — unlike invoking the
    `faceberg` console script directly, which imports faceberg.cli in
    isolation and never sees this module.
    """

    pcat = get_catalog(mode="remote", catalog_id=catalog_id)
    pcat.sync()

    if tree_view:
        _print_tree_view(catalog_id, pcat._cat)
...
def _cli() -> None:
    ...
    sync_from_hub(args.catalog_id, tree_view=args.tree_view)
